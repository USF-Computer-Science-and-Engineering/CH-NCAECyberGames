...
#     any logins not listed in /etc/vsftpd/user_list.
#
# Notes:
...
sudo sed -i "s|.*anon_upload_enable.*|anon_upload_enable=NO|g" /etc/vsftpd/vsftpd.conf
echo -e "COMPLETE\n"

# --- ALLOW CHROOT (change local root) ---
echo -e "\n[+] Enabling chroot"
sudo sed -i "s|.*chroot_local_user.*|chroot_local_user=YES|g" /etc/vsftpd/vsftpd.conf
echo -e "COMPLETE\n"
...
sudo sed -i "s|.*write_enable.*|write_enable=YES|g" /etc/vsftpd/vsftpd.conf
echo -e "COMPLETE\n"
...
sudo echo "local_root=/mnt/Files" | sudo tee -a /etc/vsftpd/vsftpd.conf > /dev/null
echo -e "COMPLETE\n"
...
echo "[+] Enabling full FTP logging in /etc/vsftpd/vsftpd.conf"
...
echo "[+] Last five lines of /etc/vsftpd/vsftpd.conf:"
sudo tail -n 5 /etc/vsftpd/vsftpd.conf
echo -e "\n"
